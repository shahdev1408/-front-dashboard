...
// Note: Your SME model needs firstName/lastName for avatar generation
interface ISME {
    _id: string;
...
const AddSmeDialog = ({ open, setOpen, onSmeAdded }: { open: boolean, setOpen: (open: boolean) => void, onSmeAdded: () => void }) => {
    const [formData, setFormData] = useState({
        ...
        phone: "",
        bio: "",
        // Note: Password field required for backend User creation logic
        password: "", 
    });
    ...
    const handleSubmit = async (e: FormEvent) => {
        ...
        try {
            ...
            const userResponse = await api.post('/users', {
                ...
                role: 'instructor' // Assume SMEs are instructors in the User table
            });

            const newUserId = userResponse.data.user._id;

            // 2. Create the SME profile (required by your frontend plan)
            const smeResponse = await api.post('/smes/add', {
                userId: newUserId,
                name: formData.name,
                expertise: formData.expertise,
                email: formData.email,
                phone: formData.phone, // Though phone/bio aren't in your SME model, we'll send them
                bio: formData.bio,
            });
            ...
            setFormData({ name: "", expertise: "", email: "", phone: "", bio: "", password: "" }); // Reset form

        } catch (err: any) {
            console.error("SME creation failed:", err);
            // Handle errors from either the /users or /smes endpoint
            const msg = err.response?.data?.msg || err.response?.data?.Error || "Server error. Check required fields.";
            setError(msg);
        ...
        <Dialog open={open} onOpenChange={setOpen}>
            <DialogContent className="glass-card max-w-2xl dark:bg-gray-800">
                <DialogHeader>
                    ...
                    <DialogDescription>
                        Enter SME details to add them to the platform. **A User account will also be created.**
                    </DialogDescription>
                </DialogHeader>
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            <Label htmlFor="phone">Temporary Password</Label> {/* Use temporary password field */}
    ...
    const fetchSmes = async () => {
        ...
        try {
            ...
            if (Array.isArray(response.data)) {
                ...
                const formattedSmes = response.data.map(sme => ({
                    ...sme,
                    courses: sme.activeCourses || 0, // Use the activeCourses field from the model
